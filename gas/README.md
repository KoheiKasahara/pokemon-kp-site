...
このコードは、GitHub ActionsがJSONを生成するときだけに使う公開用の読み取りAPIです。ブラウザから直接アクセスしません。
...
公開サイトにはテスト用の `/dev` URLではなく、デプロイ後の `/exec` URLをGitHub Actionsの `GAS_API_URL` シークレットへ設定する。
...
- GitHub ActionsがGASから取得したJSONだけをGitHub Pagesへ公開するため、ブラウザ側のCORS確認は不要。
